...
class VGG_A_BN(nn.Module):
    def __init__(self, inp_ch=3, num_classes=10, init_weights=True):
        ...
        self.features = nn.Sequential(
            ...
            nn.Conv2d(64, 128, 3, padding=1), nn.BatchNorm2d(128), nn.LeakyReLU(inplace=True), nn.MaxPool2d(2, 2),
            nn.Conv2d(128, 256, 3, padding=1), nn.BatchNorm2d(256), nn.LeakyReLU(inplace=True),
            nn.Conv2d(256, 256, 3, padding=1), nn.BatchNorm2d(256), nn.LeakyReLU(inplace=True), nn.MaxPool2d(2, 2),
            nn.Conv2d(256, 512, 3, padding=1), nn.BatchNorm2d(512), nn.LeakyReLU(inplace=True),
            nn.Conv2d(512, 512, 3, padding=1), nn.BatchNorm2d(512), nn.LeakyReLU(inplace=True), nn.MaxPool2d(2, 2),
    ...
    def _init_weights(self):
        for m in self.modules():
            init_weights_(m)
# VGG with BatchNorm + Dropout
class VGG_A_BN_Dropout(nn.Module):
    def __init__(self, inp_ch=3, num_classes=10, init_weights=True):
        super().__init__()
        ...
        self.classifier = nn.Sequential(
            nn.Dropout(),                 # Dropout 层 1
            nn.Linear(512, 512),
            nn.ReLU(True),
            nn.Dropout(),                 # Dropout 层 2
            nn.Linear(512, 512),
            nn.ReLU(True),
            nn.Linear(512, num_classes)
        )
